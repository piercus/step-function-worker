...
* @returns {Promise}
*/
Worker.prototype.start = async function () {
	this.increasePool();
...
Worker.prototype.addTask = function (parameters) {
	// This.logger.count('addTask');
	const task = new Task({...parameters, worker: this, logger: this.logger});
...
Worker.prototype.close = async function (cb) {
	...
	if (!this._stoppingPromise) {
		this._stoppingPromise = new Promise((resolve, reject) => {
			const onEmpty = () => {
				...
				if (this._tasks.length > 0) {
					const error = new Error('Some tasks are still ongoing, please make sure all the tasks are finished before stopping the worker');
					return reject(error);
				}
...
Worker.prototype.restart = async function (cb) {
